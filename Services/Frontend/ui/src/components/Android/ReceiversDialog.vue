      ...
      <div class="d-flex flex-row">
        <v-tabs v-model="localTab" background-color="primary" direction="vertical">
          <v-tab prepend-icon="mdi-export" value="exported">
            Exported
            <v-chip size="x-small" class="ml-1" color="error" variant="tonal">{{ receivers.exported.length }}</v-chip>
          </v-tab>
          <v-tab prepend-icon="mdi-lock-outline" value="non_exported">
            Non-Exported
            <v-chip size="x-small" class="ml-1" variant="tonal">{{ receivers.non_exported.length }}</v-chip>
          </v-tab>
        </v-tabs>
        ...
        <v-window v-model="localTab">
          <!-- ── Exported ── -->
          <v-window-item value="exported">
            <v-card flat>
              <v-card-text>
                <div class="mb-2 text-caption text-medium-emphasis">
                  Showing {{ filteredExportedReceivers.length }} of {{ receivers.exported.length }} exported receivers
                ...
                <div v-if="filteredExportedAll.length === 0" class="text-center pa-6 text-medium-emphasis">
                  ...
                  <div>No receivers match the current filter.</div>
                </div>

                <v-expansion-panels v-else variant="accordion">
                  <v-expansion-panel v-for="receiver in filteredExportedReceivers" :key="receiver.name">
                    <v-expansion-panel-title :expand-icon="hasDetails(receiver) ? undefined : ''">
                      <div class="d-flex align-center w-100 gap-2" style="min-width:0;">
                        <v-btn variant="text" icon size="small" @click.stop="getFileAndReceiver(receiver)" :disabled="!decompiledReceivers.has(receiver.name)">
                          <v-icon size="18" :color="decompiledReceivers.has(receiver.name) ? 'primary' : 'grey'">mdi-xml</v-icon>
                        </v-btn>
                        <v-tooltip :text="receiver.name" location="top">
                          <template v-slot:activator="{ props }">
                            <span v-bind="props" class="component-name text-body-2">{{ receiver.name }}</span>
                          </template>
                        </v-tooltip>
                        <v-spacer></v-spacer>
                        <div class="d-flex align-center gap-1 flex-shrink-0">
                          <v-chip v-if="hasIntentFilters(receiver)" size="x-small" color="primary" variant="tonal">
                            <v-icon start size="12">mdi-filter-variant</v-icon>
                            {{ receiver.intentFilters.length }}
                          </v-chip>
                        </div>
                      </div>
                    </v-expansion-panel-title>
                    <v-expansion-panel-text v-if="hasDetails(receiver)">
                      <v-expansion-panels variant="accordion" class="mt-2">
                        <v-expansion-panel v-if="hasIntentFilters(receiver)">
                          <v-expansion-panel-title>
                            <v-icon class="mr-2" size="18">mdi-filter-variant</v-icon>
                            Intent Filters ({{ receiver.intentFilters.length }})
                          </v-expansion-panel-title>
                          <v-expansion-panel-text>
                            <intent-filters :filters="receiver.intentFilters"></intent-filters>
                          </v-expansion-panel-text>
                        </v-expansion-panel>
                        <v-expansion-panel v-if="receiver.manifestSnippet">
                          <v-expansion-panel-title>
                          ...
                          <v-expansion-panel-text>
                            <div v-highlight>
                              <pre class="language-xml manifest-code"><code>{{ receiver.manifestSnippet }}</code></pre>
                            </div>
                          </v-expansion-panel-text>
                        </v-expansion-panel>
                      </v-expansion-panels>
          ...
          <v-window-item value="non_exported">
            <v-card flat>
              <v-card-text>
                <div class="mb-2 text-caption text-medium-emphasis">
                  Showing {{ filteredNonExportedReceivers.length }} of {{ receivers.non_exported.length }} non-exported receivers
                  <span v-if="quickFilter !== 'all'"> · filtered by <strong>{{ activeFilterLabel }}</strong></span>
                </div>
                ...
                <v-expansion-panels v-else variant="accordion">
                  <v-expansion-panel v-for="receiver in filteredNonExportedReceivers" :key="receiver.name">
                    <v-expansion-panel-title :expand-icon="hasDetails(receiver) ? undefined : ''">
                      <div class="d-flex align-center w-100 gap-2" style="min-width:0;">
                        <v-btn variant="text" icon size="small" @click.stop="getFileAndReceiver(receiver)" :disabled="!decompiledReceivers.has(receiver.name)">
                          <v-icon size="18" :color="decompiledReceivers.has(receiver.name) ? 'primary' : 'grey'">mdi-xml</v-icon>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const receivers = ref({ exported: [], non_exported: [] });
    const decompiledReceivers = ref(new Set());
    ...
    const hasIntentFilters = (r) => r.intentFilters?.length > 0;
    const hasDetails = (r) => hasIntentFilters(r) || !!r.manifestSnippet;
    const receiverPriority = (r) => hasIntentFilters(r) ? 1 : 0;
    ...
    function buildFiltered(list) {
      ...
      return [...result].sort((a, b) => {
        const pd = receiverPriority(b) - receiverPriority(a);
        return pd !== 0 ? pd : a.name.localeCompare(b.name);
      });
    }

    const filteredExportedAll = computed(() => buildFiltered(receivers.value.exported));
    const filteredNonExportedAll = computed(() => buildFiltered(receivers.value.non_exported));

    const filteredExportedReceivers = computed(() => {
      const start = (currentPage.value - 1) * itemsPerPage.value;
      return filteredExportedAll.value.slice(start, start + itemsPerPage.value);
    });
    const filteredNonExportedReceivers = computed(() => {
      const start = (currentPage.value - 1) * itemsPerPage.value;
    ...
    const quickFilters = computed(() => {
      const src = localTab.value === 'exported' ? receivers.value.exported : receivers.value.non_exported;
      return [
    ...
    const getFileAndReceiver = async (receiver) => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_APP_API_URL}/engine/decompiled/${props.filename}/${receiver.name}`);
        if (response.data?.java_code) {
          await store.dispatch('setCodeViewerData', { code: response.data.java_code, filename: props.filename, componentName: receiver.name });
          props.openCodeViewer();
        } else {
          showSnackbar('No code available for this receiver.', 'warning');
        }
      } catch {
    ...
    const checkBatchStatus = async (names) => {
      try {
        ...
        if (response.data?.status) {
          ...
          decompiledReceivers.value = confirmed;
        }
    ...
    const fetchReceivers = async () => {
      ...
      decompiledReceivers.value = new Set();
      try {
        const response = await axios.get(`${import.meta.env.VITE_APP_API_URL}/audit/receivers/${props.filename}`);
        receivers.value = response.data;
        const names = [...receivers.value.exported, ...receivers.value.non_exported].map(r => r.name);
        if (names.length) await checkBatchStatus(names);
      } catch {
        showSnackbar('Error fetching receivers', 'error');
      }
    };

    onMounted(fetchReceivers);
    watch(() => props.filename, fetchReceivers);
    watch(() => props.dialog, (v) => { if (v) fetchReceivers(); });
    ...
    return {
      ...
      closeDialog, getFileAndReceiver,
      isDark: computed(() => store.state.isDark),
      receivers, decompiledReceivers,
      filteredExportedAll, filteredNonExportedAll,
      filteredExportedReceivers, filteredNonExportedReceivers,
